...
It's 1.1GB on disk as a gzipped tsv. Let's try to read it in and time how long it takes:
...
system.time({x <- fread("/n/data1/hms/ccb/resources/git_backed_test/data/x.tsv.gz")})

x
```

For comparison that `fread()` takes 22.1 seconds on an O2 compute node using 1 thread.
