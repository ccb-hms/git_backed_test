...
Are  directories only readable by the `hpc_ccb` group readable? I ran `chmod -R 770` on this directory:

```{r}
file.access("/n/data1/hms/ccb/resources/git_backed_test/chmod_test/test.txt",
			mode = 2) # write permission
file.access("/n/data1/hms/ccb/resources/git_backed_test/chmod_test/test.txt",
			mode = 4) # read permission
```

## DB connections

Can it read from a SQLite db on O2?
...
# # Setup
# dat = data.frame(id = paste("x", 1:1e7, sep = ""),
#                 x = rnorm(1e7)) |>
#     collapse::roworderv(neworder = sample.int(1e7))
# 
# mdb = dbConnect(RSQLite::SQLite(), "/n/data1/hms/ccb/resources/git_backed_test/tmp.sqlite")
# 
# dbWriteTable(mdb, "dat", dat)
# 
# dbExecute(mdb, "CREATE INDEX idx_id ON dat(id)")
# 
# datdb = tbl(mdb, "dat")
# 
# head(datdb) |> collect()
...
mdb = dbConnect(RSQLite::SQLite(),
                "/n/data1/hms/ccb/resources/git_backed_test/tmp.sqlite")
